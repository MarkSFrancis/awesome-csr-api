...
- Consider keeping the variables that Cloudflare will need consistent (e.g. all preview releases use the same variable values). If they're dynamic (e.g. they change on every PR deployment), you'll likely need to opt-out of Cloudflare's built-in `git` integration and use [wrangler deploy](https://developers.cloudflare.com/pages/get-started/direct-upload/#deploy-your-assets) instead, so that you can inject any generated values from your resource(s) into the Cloudflare `wrangler.toml` file before the app is deployed to Cloudflare
